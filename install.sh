...
CONTAINERS=(faraday-db faraday-redis faraday-web faraday-worker)
HEALTH_TIMEOUT=120
...
print_step "Pinning bcrypt inside containers"
for service in faraday worker; do
  if $COMPOSE -f "$PROJECT_DIR/docker-compose.yaml" exec -T "$service" pip install 'bcrypt<4' >/dev/null; then
    echo "  - $service: bcrypt downgraded to <4"
  else
    print_warn "could not pin bcrypt for $service (continuing)"
  fi
done
